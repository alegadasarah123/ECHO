...
const SOSScreen = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        // Extract user ID
        let extractedUserId = parsedUserData.user_id || parsedUserData.id || parsedUserData.profile?.id;
        
        if (extractedUserId) {
          setUserId(extractedUserId);
          console.log("[DEBUG] ✅ Found user ID:", extractedUserId);
        } else {
          console.log("[DEBUG] ❌ No user ID found in user data");
        ...
        // Try to auto-fill contact number
        const phone = parsedUserData.phoneNumber || parsedUserData.phone_number || parsedUserData.phone;
        if (phone) {
          setContactNumber(phone);
          console.log("[DEBUG] ✅ Auto-filled contact number:", phone);
        }

        // Try to auto-fill user name
        const firstName = parsedUserData.firstName || parsedUserData.first_name || "";
        const lastName = parsedUserData.lastName || parsedUserData.last_name || "";
  ...
  const handleSendSOS = async () => {
    ...
    if (horseStatus.length === 0) {
      Alert.alert("Error", "Please select at least one horse status.");
      return;
    }
    ...
    if (currentLocation === 'Tap to get current location') {
  ...
  // Show loading screen while loading user profile
  if (isLoadingProfile) {
    return (
  ...
  return (
    <View style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor="#DC3545" translucent={false} />
...
const styles = StyleSheet.create({
  ...
  statusButtonActive: { backgroundColor: "#DC3545", borderColor: "#DC3545" },
  ...
  locationDot: { width: scale(8), height: scale(8), borderRadius: scale(4), backgroundColor: "#DC3545" },
