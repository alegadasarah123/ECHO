...
const NotificationsModal = ({ isOpen, onNotificationClick, onClose, onMarkAllAsRead }) => {
  ...
  const fetchNotifications = useCallback(async (force = false) => {
    ...
    try {
      ...
      console.log("Fetching notifications from DVMF API...");
      ...
      const data = await response.json();
      console.log("DVMF Notifications received:", data.length);
  ...
  useEffect(() => {
    if (isOpen) {
      console.log("DVMF Modal opened, fetching notifications...");
            ...
            <>
                  ...
                  <div style={styles.notificationContent}>
                    {!n.read && (
                      <div style={styles.unreadIndicatorContainer}>
                        <Circle size={8} fill="#0F3D5A" color="#0F3D5A" />
                      </div>
                    )}
              ...
              {loading && (
                <div style={styles.loadingMoreContainer}>
                  <Loader2 size={16} color="#0F3D5A" className="spinner" />
...
const styles = {
  ...
  clearButton: {
    ...
    borderRadius: "8px",
    transition: "all 0.2s ease",
    boxShadow: "0 2px 4px rgba(15, 61, 90, 0.2)",
    display: "flex",
    alignItems: "center",
    gap: "6px",
  },
