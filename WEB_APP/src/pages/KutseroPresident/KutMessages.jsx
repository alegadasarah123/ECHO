...
const formatAddress = (data, userType = 'kutsero') => {
  if (userType === 'kutsero') {
    const parts = [data.kutsero_brgy, data.kutsero_municipality, data.kutsero_city, data.kutsero_province].filter(Boolean);
    return parts.length > 0 ? `${parts.join(', ')} ${data.kutsero_zipcode || ''}`.trim() : 'Address not available';
  } else if (userType === 'horse_operator') {
    const parts = [data.op_house_add, data.op_brgy, data.op_municipality || data.op_city, data.op_province].filter(Boolean);
    return parts.length > 0 ? `${parts.join(', ')} ${data.op_zipcode || ''}`.trim() : 'Address not available';
  }
...
const KutseroProfileModal = ({ user, profileData, isOpen, onClose }) => {
  ...
  const cleanName = user.name ? user.name.replace(/\s*\([^)]*\)\s*$/, '').trim() : 'Unknown Kutsero';
  ...
  const address = formatAddress(profileData, 'kutsero');
...
const HorseOperatorProfileModal = ({ user, profileData, isOpen, onClose }) => {
  ...
  const cleanName = user.name ? user.name.replace(/\s*\([^)]*\)\s*$/, '').trim() : 'Unknown Horse Operator';
  ...
  const address = formatAddress(profileData, 'horse_operator');
...
>>>>>>> Stashed changes
    ...
    return (
      <div className="fixed inset-0 z-1001 flex items-center justify-center backdrop-blur-sm p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#D2691E] mx-auto"></div>
  ...
  // Handle Kutsero and Horse Operator profiles
  if (user.role === 'Kutsero') {
    return (
      <KutseroProfileModal
        user={user}
  ...
  return (
    <div className={`flex ${isFullscreen ? 'items-center gap-2' : 'flex-col'}`}>
      <span 
        ...
        className={`font-semibold text-gray-900 ${shouldBeClickable ? 'hover:text-[#D2691E] cursor-pointer transition-colors' : ''}`}
      >
        {cleanName}
      </span>
...
const Avatar = ({ user, size = "md", onClick, clickable = true, inChatView = false }) => {
  ...
  const avatarContent = predefinedAvatar ? (
    ...
    <div
      className={`${sizeClasses[size]} bg-gradient-to-br from-[#D2691E] to-[#8B4513] rounded-full flex items-center justify-center text-white font-semibold ${
        shouldBeClickable ? "cursor-pointer hover:opacity-80 transition-opacity" : ""
      }`}
      onClick={shouldBeClickable ? onClick : undefined}
    >
    ...
    return (
      <div className="flex justify-center my-8">
        <div className="text-center">
          <div className="bg-[#D2691E]/10 border border-[#D2691E]/20 rounded-lg p-6 max-w-md">
            <MessageCircle className="w-12 h-12 text-[#D2691E] mx-auto mb-3" />
              ...
              >
                <div
                  className={`px-4 py-2 rounded-2xl text-sm ${
                    message.isOwn
                      ? "bg-[#D2691E] text-white rounded-br-md"
                ...
                {message.isOwn && 
                 ...
                 message.is_read === true && (
                  <div className="text-right mt-1">
                    <span className="text-xs text-[#D2691E] font-medium">Seen</span>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 p-4 hover:bg-gray-50 cursor-pointer border-b border-gray-100 transition-colors ${
        isSelected ? 'bg-[#D2691E]/10 border-[#D2691E]/20' : ''
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            {hasUnread && (
              <div className="w-5 h-5 bg-[#D2691E] text-white rounded-full flex items-center justify-center text-xs font-medium">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
              newMessage.trim()
                ? "bg-[#D2691E] hover:bg-[#8B4513] text-white"
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#D2691E]"
          />
        </div>
      </div>
    ...
    return (
      <>
        <div className="fixed bottom-6 right-6 z-1000 w-80 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          {!selectedConversation && (
            <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-[#D2691E] to-[#8B4513] text-white">
              <div className="flex items-center gap-2">
